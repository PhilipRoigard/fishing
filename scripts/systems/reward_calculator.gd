...
static func calculate_coin_reward(fish: FishData, rod_quality_bonus: float = 1.0) -> int:
	...
	var final_value: float = base_value * rod_quality_bonus
	return maxi(1, roundi(final_value))


static func calculate_xp_reward(caught_quality: int) -> int:
	var reward_config: RewardConfig = _get_reward_config()
	if reward_config:
		return reward_config.get_xp_for_rarity(caught_quality)
	return 10
